...
function initialize_popup() {
    ...
    window.addEventListener("keydown", (event) => {
        if (event.key == "Escape" || (KeyboardUtilities.isPrimaryPressed(event) && event.altKey && event.key=="c")) {
            event.preventDefault();
            cancel_clicked();
        } else if(KeyboardUtilities.isPrimaryPressed(event) && event.altKey && event.key=="o") {
            event.preventDefault();
            ok_clicked();
        } else if(KeyboardUtilities.isPrimaryPressed(event) && event.altKey && event.key=="a") {
            event.preventDefault();
            apply_clicked();
        }
    });
...
function initialize_main(production_status) {
    if (navigator.onLine && !production_status) {
        window.addEventListener("keydown", (event) => {
            if (KeyboardUtilities.isPrimaryPressed(event) && event.altKey && event.key=="e") {
                settings_window = open_popup(); // weird Firefox browser error: popup blocker when triggered by non-mouse event (e.g. keyboard here)
                event.preventDefault();
            }  else if (KeyboardUtilities.isPrimaryPressed(event) && event.altKey && event.key=="h") {
                show_help();
                event.preventDefault();
            }
        });
